...
def resolve_scapy_iface(local_ip: str | None = None) -> str:
    """Retourne l'interface Npcap/Scapy correspondant à l'IP locale active."""
    from scapy.all import get_if_addr, get_if_list  # type: ignore[import-untyped]
    from scapy.config import conf  # type: ignore[import-untyped]

    if local_ip is None:
        from app.scanner.device_scanner import _get_local_network

        local_ip = _get_local_network().ip
...
def get_iface_hardware_mac(iface: str) -> str:
    """MAC matérielle de l'interface réseau (format AA:BB:CC:DD:EE:FF)."""
    from scapy.all import get_if_hwaddr  # type: ignore[import-untyped]

    return get_if_hwaddr(iface).upper().replace("-", ":")
