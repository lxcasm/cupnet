    ...
    def block(self, ip: str, mac: str) -> None:
        ...
        for direction in ("in", "out"):
            ...
            if result.returncode != 0:
                errors.append(result.stderr.strip() or result.stdout.strip() or f"code {result.returncode}")

        if errors:
            raise RuntimeError(
                "Impossible de créer la règle pare-feu Windows.\n"
                "Lancez CupNet en administrateur.\n\n"
                + "\n".join(errors[:2])
            )
